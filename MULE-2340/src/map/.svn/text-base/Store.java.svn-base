...
public class Store implements Serializable {
	...
	/**
	 * Sets the store resources based on difficulty.
	 */
	public void setStockResources() {
		if (GameData.getInstance().getDifficulty().equals("Easy")) {
			stockFood = 16;
			stockEnergy = 16;
			stockOre = 0;
		} else if (GameData.getInstance().getDifficulty().equals("Standard")
				|| GameData.getInstance().getDifficulty().equals("Hard")) {
			stockFood = 8;
			stockEnergy = 8;
			stockOre = 8;
		}
	...
	/**
	 * When the player is in the store
	 * 
	 * @param gc
	 *            Current game container
	 * @param leftClick
	 *            Was there a click.
	 * @return player still in store?
	 */
	public boolean onclick(GameContainer gc, boolean leftClick) {
		Player currentPlayer = GameData.getInstance().getActivePlayer();
		...
		if (leftClick) {
			...
			if (submitB.checkClick(gc.getInput().getMouseX(), gc.getInput()
				...
				if ((goldLeft >= 0)
						&& (Integer.parseInt(tff1.getText()) <= stockFood)
					...
					stockFood -= Integer.parseInt(tff1.getText());
					currentPlayer
							.boughtEnergy(Integer.parseInt(tfe1.getText()));
					...
					if (currentPlayer
							.soldFood(Integer.parseInt(tff2.getText())))
						stockFood += Integer.parseInt(tff2.getText());
					if (currentPlayer.soldEnergy(Integer.parseInt(tfe2
							.getText())))
						stockEnergy += Integer.parseInt(tfe2.getText());
					if (currentPlayer.soldOre(Integer.parseInt(tfo2.getText())))
			...
			if (buyFMule.checkClick(gc.getInput().getMouseX(), gc.getInput()
				...
				if (GameData.getInstance().getActivePlayer().deductGold(125)) {

					GameData.getInstance().getActivePlayer().buyMule(Mule.FOOD); // Food
																					// Mule
					return false;
				}
			}
			if (buyEMule.checkClick(gc.getInput().getMouseX(), gc.getInput()
				...
				if (GameData.getInstance().getActivePlayer().deductGold(150)) {
					GameData.getInstance().getActivePlayer()
							.buyMule(Mule.ENERGY); // Energy
					// Mule
					return false;
				}
			}
			if (buyOMule.checkClick(gc.getInput().getMouseX(), gc.getInput()
				...
				if (GameData.getInstance().getActivePlayer().deductGold(175)) {
					GameData.getInstance().getActivePlayer().buyMule(Mule.ORE); // Ore
																				// mule
					return false;
				}
			}
	...
	/**
	 * Draws the inside of the store to the screen
	 * 
	 * @param g
	...
	public void drawStoreUI(Graphics g, GameContainer gc) {
		...
		if (currentPlayer.returnMuleEquipped() || currentPlayer.getGold() < 175) {

			buyFMule.disable();
			buyEMule.disable();
			buyOMule.disable();
			if (!currentPlayer.returnMuleEquipped()) {
				if (currentPlayer.getGold() >= 125)
					buyFMule.enable();
				if (currentPlayer.getGold() >= 150)
					buyEMule.enable();
				if (currentPlayer.getGold() >= 175)
					buyOMule.enable();
			}
		}
		...
		cancelB.drawButton(g, Color.blue);
		submitB.drawButton(g, Color.blue);
		buyFMule.drawButton(g, Color.blue);
		buyEMule.drawButton(g, Color.blue);
		buyOMule.drawButton(g, Color.blue);
	...
	/**
	 * Is the point within the store bounds.
	 * 
	 * @param pX
	 *            Player Location X
	 * @param pY
	 *            Player Location Y
	 * @return if the player is in the store bounds
	 */
	...
	/**
	 * Draw store image to town screen.
	 * 
	 * @param g
	 */
