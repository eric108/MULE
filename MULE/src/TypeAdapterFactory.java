...
class RaceTypeAdapterFactory implements TypeAdapterFactory {
	...
	public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
		...
		/*
		 * Lookup type adapters to do the actual work. We use getNextAdapter to
		 * avoid getting 'this' on the types that this factory supports.
		 */
		final TypeAdapter<Race> raceAdapter = gson.getDelegateAdapter(this,
				TypeToken.get(Race.class));
		final TypeAdapter<Human> humanAdapter = gson.getDelegateAdapter(this,
				TypeToken.get(Human.class));
		final TypeAdapter<Bonzoid> bonzoidAdapter = gson.getDelegateAdapter(
				this, TypeToken.get(Bonzoid.class));
		final TypeAdapter<Buzzite> buzziteAdapter = gson.getDelegateAdapter(
				this, TypeToken.get(Buzzite.class));
		final TypeAdapter<Flapper> flapperAdapter = gson.getDelegateAdapter(
				this, TypeToken.get(Flapper.class));
		final TypeAdapter<Ugaite> ugaiteAdapter = gson.getDelegateAdapter(this,
				TypeToken.get(Ugaite.class));
		/*
		 * The JsonElement type adapter is always handy when we want to tweak
		 * what our delegate type adapter created.
		 */
		...
		TypeAdapter<Race> result = new TypeAdapter<Race>() {
			...
			public void write(JsonWriter out, Race value) throws IOException {
				if (value instanceof Bonzoid) {
					...
					JsonObject object = raceAdapter.toJsonTree(value)
							.getAsJsonObject();
			...
			public Race read(JsonReader in) throws IOException {
				/*
				 * Use the appropriate type adapter based on the contents of the
				 * stream.
				 */
