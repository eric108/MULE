 ...
 * @author Ziyi Jiang
 * 
 * int playerCount represents the number of players chosen.
 * String difficulty represents the difficulty chosen.
 * boolean randomMap represents if a random map is desired.
 *
 */
...
public class GameOptions extends BasicGameState {
	...
	public void update(GameContainer container, StateBasedGame sbg, int arg2)
		...
		if(container.getInput().isMousePressed(Input.MOUSE_LEFT_BUTTON)){
			...
			//Player Count
			if(upPC.checkClick(container.getInput().getMouseX(),
					container.getInput().getMouseY()) && (playerCount < 4))
				playerCount++;
			...
			//Difficulties
			if(upDif.checkClick(container.getInput().getMouseX(),
					container.getInput().getMouseY()) && (difficulty.equals("Easy")))
				difficulty = "Standard";
			else if(upDif.checkClick(container.getInput().getMouseX(),
					container.getInput().getMouseY()) && (difficulty.equals("Standard")))
				difficulty = "Hard";
			else if(downDif.checkClick(container.getInput().getMouseX(),
					container.getInput().getMouseY()) && (difficulty.equals("Standard")))
				difficulty = "Easy";
			else if(downDif.checkClick(container.getInput().getMouseX(),
					container.getInput().getMouseY()) && (difficulty.equals("Hard")))
				difficulty = "Standard";
				
			//Random Map
			if(randBox.checkClick(container.getInput().getMouseX(),
					container.getInput().getMouseY()) && !(randomMap)){ //not selected
				randomMap = true;
				rand = "yes";
			...
			{
				gd = new GameData(playerCount, randomMap, difficulty);
	...
	/**
	 * unique state identifier that is used to determined if the state will be displayed
	 */
	public int getID() {
